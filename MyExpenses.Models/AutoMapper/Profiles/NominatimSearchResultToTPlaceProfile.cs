...
public class NominatimSearchResultToTPlaceProfile : Profile
{
    public NominatimSearchResultToTPlaceProfile()
    {
        CreateMap<NominatimSearchResult, TPlace>()
            ...
            .ForMember(dest => dest.Number, opt => opt.MapFrom(src => src.Address != null ? src.Address.HouseNumber : null))
            .ForMember(dest => dest.Street, opt => opt.MapFrom(src => src.Address != null ? src.Address!.Road ?? src.Address!.Suburb : null))
            .ForMember(dest => dest.Postal, opt => opt.MapFrom(src => src.Address != null ? src.Address!.Postcode : null))
