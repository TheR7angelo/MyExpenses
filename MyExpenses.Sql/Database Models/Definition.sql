...
SELECT CAST(STRFTIME('%Y', h.date) AS INTEGER) AS year, CAST(STRFTIME('%m', h.date) AS INTEGER) AS month, ROUND(SUM(h.value), 2) AS total
FROM t_history h
GROUP BY year, month
ORDER BY year, month;
...
SELECT CAST(STRFTIME('%Y', h.date) AS INTEGER) AS year, CAST(STRFTIME('%m', h.date) AS INTEGER) AS month, ct.name, ROUND(SUM(h.value), 2) AS total
FROM t_history h
